...
metric definitions. Do not invent your own logic. If a question cannot be answered
with these metrics, set METRIC_USED to NONE.

Table: `bigquery-public-data.thelook_ecommerce.order_items`

APPROVED METRICS:
1. net_revenue: SUM(CASE WHEN status NOT IN ('Cancelled','Returned') THEN sale_price ELSE 0 END)
2. gross_revenue: SUM(CASE WHEN status != 'Cancelled' THEN sale_price ELSE 0 END)
3. return_rate: COUNTIF(status='Returned') / COUNTIF(status != 'Cancelled') * 100
4. active_customers: COUNT(DISTINCT CASE WHEN status != 'Cancelled' THEN user_id END)

DATE RULE: anchor to created_at; use half-open ranges (>= start AND < end);
never use BETWEEN; never use CURRENT_DATE() unless the user explicitly says "today".
"""
...
def evaluate(bq, claude, question):
    ...
    mapped = metric and metric.upper() not in ("NONE", "UNKNOWN")
    if mapped:
        score += 40
        reasons.append((True, f"Mapped to approved metric: {metric}", "+40"))
    else:
        reasons.append((False, "No approved metric matched — answer is ungoverned", "+0"))
    ...
    return {
        ...
        "metric": metric, "gov_val": gov_val, "gov_sql": gov_sql,
...
examples = {
    "Return rate": "What's our return rate?",
    "Active customers": "How many active customers do we have?",
    "Jan 2025 revenue": "What was total revenue in January 2025?",
}
...
if (go or clicked) and question:
    ...
    pill = f'<div class="metric-pill">{res["metric"]}</div>' if res["metric"].upper() not in ("NONE","UNKNOWN") else ""
    st.markdown(f"""
    <div class="answer-card">
        <div class="answer-label">Answer</div>
        <div class="answer-value">{fmt(res["gov_val"])}</div>
        {pill}
    </div>
    """, unsafe_allow_html=True)

    # Raw value framed as evidence, not a competing answer
    raw_differs = True
    if is_num(res["raw_val"]) and is_num(res["gov_val"]) and res["gov_val"] != 0:
        raw_differs = abs(res["raw_val"] - res["gov_val"]) / abs(res["gov_val"]) > 0.01
    ...
    if raw_differs:
        ...
        <div class="evidence-card">
            ...
            <b>{fmt(res["raw_val"])}</b> for this question — a different number using assumptions no one approved.
            That gap is why this answer is flagged for review rather than fully trusted.</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
    ...
    with st.expander("View generated SQL"):
        st.markdown(f'<div class="answer-label">Raw path</div><div class="sql-block">{res["raw_sql"]}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="answer-label" style="margin-top:0.8rem;">Governed path</div><div class="sql-block">{res["gov_sql"]}</div>', unsafe_allow_html=True)
